...
describe("번역 초안 end-to-end 계약", () => {
  ...
  it("Linux worker는 API key가 아닌 linux-coder OAuth model로 claim·retry·timeout·cleanup한다", async () => {
    ...
    expect(helper).toContain("HERMES_OAUTH_ENVIRONMENT_ALLOWLIST");
    expect(helper).toContain("stdin: prompt");
    expect(helper).toContain("hermesOAuthBridgeArgs(invocation, source)");
    expect(helper).toContain("listHermesSourceSessions");
